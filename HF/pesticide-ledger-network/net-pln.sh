...
function printHelp() {
  ...
  echo "      - 'deploySmartContract' - deploy the pesticideledger chaincode on the channel"
...
function createConsortium() {
  ...
  set -x
  configtxgen -profile PesticideLedgerOrdererGenesis -channelID system-channel -outputBlock ./system-genesis-block/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts log.txt pesticideledgernetwork.tar.gz
